...
def _extract_steps(events: list[dict]) -> list[dict]:
    """Build list of step dicts: index, state, action, levels_completed, PIL Image, and nav_path."""
    steps = []
    current_nav_path = None  # most recent navigate_to path
    for ev in events:
        data = ev.get("data") or {}
        # Track nav_path events from navigate_to calls
        if isinstance(data, dict) and data.get("event") == "nav_path":
            current_nav_path = {
                "start": data.get("start"),
                "end": data.get("end"),
                "path_cells": data.get("path_cells"),
        ...
        steps.append(
            {
                ...
                "img": img,
                "nav_path": current_nav_path,
            }
        )
        # Clear nav_path after the path is fully consumed (no more plan steps)
        # Keep it active while plan actions are being executed
        if current_nav_path and action_name in ("RESET", "ACTION5", "ACTION6"):
            current_nav_path = None
    return steps


def _draw_nav_path(img, nav_path: dict, scale: int = _SCALE) -> None:
    """Draw navigate_to path, start, and end markers on the grid image."""
    try:
    ...
    if not nav_path:
        return
    path_cells = nav_path.get("path_cells")
    start = nav_path.get("start")
    end = nav_path.get("end")
    if not path_cells or len(path_cells) < 2:
        return

    draw = ImageDraw.Draw(img)

    # Draw path line segments (cyan, semi-thick)
    for i in range(len(path_cells) - 1):
        r1, c1 = path_cells[i]
        r2, c2 = path_cells[i + 1]
        # Center of each 5x5 quantized cell
        x1 = int((c1 + 2.5) * scale)
        y1 = int((r1 + 2.5) * scale)
        x2 = int((c2 + 2.5) * scale)
        y2 = int((r2 + 2.5) * scale)
    ...
    if start:
        sx = int((start[1] + 2.5) * scale)
        sy = int((start[0] + 2.5) * scale)
        r = 4
        draw.ellipse([sx - r, sy - r, sx + r, sy + r], fill=(0, 255, 0), outline=(0, 200, 0))

    # Draw end/target marker (red circle)
    if end:
        ex = int((end[1] + 2.5) * scale)
        ey = int((end[0] + 2.5) * scale)
        r = 4
        draw.ellipse([ex - r, ey - r, ex + r, ey + r], fill=(255, 0, 0), outline=(200, 0, 0))

    # Draw path waypoint dots (small cyan dots)
    for cell in path_cells[1:-1]:
        cx = int((cell[1] + 2.5) * scale)
        cy = int((cell[0] + 2.5) * scale)
        draw.ellipse([cx - 2, cy - 2, cx + 2, cy + 2], fill=(0, 255, 255))


def _draw_overlay(img, step: dict) -> None:
    """Draw step/state/action/levels as text bar at bottom; ensure width fits text.
    Also draw navigate_to path overlay if present."""
    try:
        from PIL import Image, ImageDraw, ImageFont
    except ImportError:
        return

    # Draw nav path on the grid image first (before expanding canvas)
    nav_path = step.get("nav_path")
    if nav_path:
        _draw_nav_path(img, nav_path)

    w, h = img.size
    bar_h = 22
    nav_info = ""
    if nav_path and nav_path.get("end"):
        end = nav_path["end"]
        n_steps = len(nav_path.get("path_actions", []))
        nav_info = f"  |  Nav: ({end[0]},{end[1]}) {n_steps}steps"
    text = f"Step {step['index']}  |  State: {step['state']}  |  Action: {step['action']}  |  Levels: {step['levels_completed']}{nav_info}"
    try:
