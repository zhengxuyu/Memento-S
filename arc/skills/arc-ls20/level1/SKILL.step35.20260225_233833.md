...
5. When the patterns match, a large-scale change will occur where the entirely 7x7 color 5 barrier surrounding the Goal will magically disappear (turning into passable terrain like color 0).
6. Pathfind your 5x5 player block to the Goal symbol now that the path is clear. Avoid color 5 obstacles.
...
8. Overlapping the goal exactly completes the current level, clears the board, and grants +1 score. If it's the final level, it ends with "level_up".
