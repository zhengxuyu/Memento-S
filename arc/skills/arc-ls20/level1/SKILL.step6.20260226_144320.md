...
- Action 3: LEFT (X -= 5)
- Action 4: RIGHT (X += 5)
- Every move costs 1 action. The timer (color 11 blocks at bottom, rows 61-62) decreases by 1 column (2 cells) every move. When the timer runs out, the episode ends.
...
4. If patterns do not match, path to the button. Move back and forth on/off the button (e.g. ACTION3 then ACTION4) to rotate the display 90 degrees CW each time until the display EXACTLY matches the goal symbol.
...
6. **Precise Overlap**: Pathfind precisely to overlap the 3x3 goal. 
   - Since movement is 5 cells per action, you will align step-by-step. The player's bounds must completely enclose/align the goal block.
