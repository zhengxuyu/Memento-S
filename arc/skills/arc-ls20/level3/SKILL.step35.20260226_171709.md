...
2. **Action Economy vs. In-Game Timer**:
   ...
   - **CRITICAL WARNING**: You have a strict *global limit* of 100 actions per episode across all levels. Collecting time pickups costs valuable actions. You should ONLY collect pickups if they are on your direct optimal path, or if absolutely necessary to prevent game-over from the timer running out. Do not take frivolous detours.
...
- **Action Economy & Timer Limit**: The episode has a hard cut-off of 100 actions *total*. Because standard movements and pathfinding consume the majority of these 100 actions, completely perfect pathfinding is absolutely necessary to clear all levels in a single run.
- **Grid Alignment**: Rely on the strict 5x5 mod-grid layout to plan routes without wasting a single move. Wall collisions waste actions! Action tracking over multiple past episodes explicitly demonstrates the hard cap at action 100.