...
# Game Mechanics
- **Player Avatar / Block**: A 5x5 block (top 2 rows color 12, bottom 3 rows color 9). Movement is 1 cell per action.
...
- **Obstacles**: Moving into blocked cells consumes actions (depleting the timer) without changing position.
...
- **Current Player Position**: Top-left is currently around Y=30, X=39. Need to path around obstacles. If DOWN is blocked (as seen by recent action consuming timer but not moving), path horizontally (e.g., RIGHT) to find a way down.